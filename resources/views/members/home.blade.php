				...
				<div id="payments" class="card white">
					<div class="card-content">
						...
						<div id="paid-info" class="row grey-text center ">
							<h5><span class="cap-{{ $status }}-text">${{ $paid }}</span> / ${{ $member['dues'] }}</h5>
							...
							@if ($paid >= $dueNow && $paid < $member['dues'])
								<h6>DUE NOW: ${{ $dueNow }} <span class="cap-green-text">(${{ sprintf('%.2f', $paid - $dueNow) }} ahead)</span></h6>
							@elseif ($paid < $dueNow)
								<h6>DUE NOW: ${{ $dueNow }} <span class="cap-red-text">(${{ sprintf('%.2f', $dueNow - $paid) }} behind)</span></h6>
							@endif
						</div>
						@if ($payments->count() > 0)
							<div class="row">
							...
							@if ($conflicts->count() > 0)
								<ul class="collection">
									@foreach ($conflicts as $c)
										...
										@elseif ($c->status == 0)
											<span title="Pending"><li class="collection-item grey lighten-2">{{ date('n/j/Y', strtotime($c->date_absent)) }}<span class="secondary-content"><i class="material-icons orange-text">schedule</i></span></li></span>
										@else
											<span title="Declined"><li class="collection-item cap-black white-text">{{ date('n/j/Y', strtotime($c->date_absent)) }}<span class="secondary-content"><i class="material-icons cap-red-text">clear</i></span></li></span>
										@endif
									@endforeach
								</ul>
							@else
